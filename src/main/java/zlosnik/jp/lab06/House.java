                            ...
                            public void run() {
                                try (InputStream input = socket.getInputStream();
                                    ...
                                    while ((text = reader.readLine()) != null) {
                                        ...
                                        SwingUtilities.invokeLater(() -> textArea.append("Received request: " + finalText + "\n"));
                                        if (command.equalsIgnoreCase("GET")) {
                                            ...
                                            SwingUtilities.invokeLater(() -> textArea.append("Sent: " + amountToSend + ", New sewage counter value: " + newSewageCounterValue + "\n"));
                                            SwingUtilities.invokeLater(() -> sewageCounterLabel.setText("Sewage Counter: " + newSewageCounterValue));
                                        } else {
                                            writer.println("Unknown request");
                                            SwingUtilities.invokeLater(() -> textArea.append("Unknown request received\n"));
                                        }
                                    }
                                } catch (IOException ex) {
                                    ex.printStackTrace();
